 ...
 * zcc_pet_to_asc() on a value returned by zcc_asc_to_pet() may not return the
 * original value passed to zcc_asc_to_pet(). For example, ASCII has CR, LF, FF
 * while PETSCII has just CR, so zcc_pet_to_asc(zcc_asc_to_pet(0x0a)) will return
 * 0x0d, not 0x0a.
 */
 ...
 * chars.
 */
static const char *zcc_host_illegal_chars =
#if defined(CBMFM_HOST_UNIX) || defined(CBMFM_HOST_APPLE)
    "/";
#else
    "/\\?%*:|\"<>";
#endif
...
{
    const char *t = zcc_host_illegal_chars;
    while (*t != '\0' && ch != *t) {
        t++;
    }
...
void zcc_asc_to_pet_str(uint8_t *pet, const char *asc, size_t n)
{
    ...
    while (i < n && asc[i] != '\0') {
        pet[i] = zcc_asc_to_pet((uint8_t)(asc[i]));
        i++;
    }
    while (i < n) {
        pet[i++] = 0x00;
...
void zcc_pet_filename_to_host(char *asc, const uint8_t *pet, const char *ext)
{
    ...
    if (lead < trail) {
        ...
        for (i = 0; i < trail - lead; i++) {
            int b = zcc_pet_to_asc(pet[lead + i]);
            asc[i] = isprint(b) && zcc_is_host_allowed_char(b)
                ? (char)b : '_';
        }
        /* terminate string */
        asc[trail - lead] = '\0';
